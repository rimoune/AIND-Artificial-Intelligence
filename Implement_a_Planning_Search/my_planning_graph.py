...
class PlanningGraph():
    ...
    def add_literal_level(self, level):
        ...
        for action in self.a_levels[level-1]:
            for effect in action.effnodes:
                node_state = effect
                states_set.add(node_state)
                action.children.add(node_state)
                node_state.parents.add(action)
        self.s_levels.append(states_set)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        mutex_flag=False
        for n1 in node_a1.action.effect_add:
            if n1 in node_a2.action.effect_rem:
                mutex_flag=True
        for n1 in node_a1.action.effect_rem:
            if n1 in node_a2.action.effect_add:
                mutex_flag=True
        for n2 in node_a2.action.effect_add:
            if n2 in node_a1.action.effect_rem:
                mutex_flag=True
        for n2 in node_a2.action.effect_rem:
            if n2 in node_a1.action.effect_add:
                mutex_flag=True
        return mutex_flag
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        mutex_flag=False
        for eff_1 in node_a1.action.effect_add:
            if eff_1 in node_a2.action.precond_neg:
                mutex_flag=True

        for eff_1 in node_a1.action.effect_rem:
            if eff_1 in node_a2.action.precond_pos:
                mutex_flag=True

        for eff_2 in node_a2.action.effect_add:
            if eff_2 in node_a1.action.precond_neg:
                mutex_flag=True

        for eff_2 in node_a2.action.effect_rem:
            if eff_2 in node_a1.action.precond_pos:
                mutex_flag=True
        ...
        for goal in self.problem.goal:
            ...
            for level in range(len(self.s_levels)):
                ...
                for state in self.s_levels[level]:
                    logger.info("State: %s",state.symbol)
                    if goal_node.__eq__(state) and state.is_pos==True and state not in reached:
                        logger.info("state is goal")
